    ...
    <>
      ...
      <section className="relative overflow-hidden bg-[linear-gradient(180deg,#070d17,#090f1b)] px-6 py-24 sm:px-10 md:py-36 lg:px-16">
        <div className="mx-auto max-w-[1480px]">
          <motion.div {...reveal} className="mx-auto max-w-4xl text-center">
            ...
            <h2 className="mt-5 font-headline text-5xl font-medium leading-[0.98] tracking-[-0.055em] text-white md:text-7xl">
            ...
            <p className="mx-auto mt-6 max-w-2xl text-lg leading-8 text-white/52">{h('immersive.worlds.description')}</p>
          </motion.div>

          <div className="mt-16 grid gap-6 xl:grid-cols-[1.14fr_0.86fr]">
            <motion.article {...reveal} className="overflow-hidden rounded-[2rem] border border-white/10 bg-[#0b1423]">
              <div className="relative grid min-h-[430px] place-items-center overflow-hidden bg-[radial-gradient(circle_at_center,rgba(0,212,200,0.14),transparent_33%),radial-gradient(circle_at_15%_75%,rgba(232,133,12,0.2),transparent_30%),#09121f]">
                ...
                <div className="relative z-10 flex h-48 w-48 flex-col items-center justify-center rounded-full border border-white/15 bg-[#0b1321]/80 text-center shadow-[0_0_90px_rgba(232,133,12,0.18)] backdrop-blur-xl">
                  ...
                  <strong className="mt-4 font-label text-xl font-medium text-white">Beat Breeze</strong>
              ...
              <div className="p-7 sm:p-10">
                ...
                <h3 className="mt-3 font-headline text-4xl font-medium tracking-[-0.045em] text-white sm:text-5xl">Beat Breeze</h3>
                <p className="mt-4 max-w-2xl text-lg leading-8 text-white/52">{h('twoSolutions.bb.tagline')}</p>
            ...
            <motion.article {...reveal} transition={{ duration: 0.65, delay: 0.08 }} className="overflow-hidden rounded-[2rem] border border-[#d3baff]/25 bg-[#180424]">
              <div className="relative min-h-[430px] overflow-hidden">
                <Image src="/images/product-syb-hero.webp" alt="" fill sizes="(max-width:1280px) 100vw, 43vw" className="object-cover opacity-45 saturate-[0.8]" />
                <div className="absolute inset-0 bg-[linear-gradient(120deg,rgba(24,4,36,0.98)_0%,rgba(24,4,36,0.76)_50%,rgba(161,139,250,0.16)_100%)]" />
                <div className="absolute left-8 top-8 z-10 font-label text-5xl font-semibold tracking-[-0.055em] text-[#eee6ff] sm:text-6xl">Soundtrack</div>
                <div className="absolute bottom-8 left-8 z-10 flex aspect-square w-48 flex-col justify-center rounded-full border border-[#eee6ff]/30 bg-[#a18bfa]/15 p-7 backdrop-blur-xl motion-safe:animate-pulse-slow">
                  <span className="font-label text-[10px] uppercase tracking-[0.18em] text-[#c9b2ff]">{h('immersive.worlds.sybDiscLabel')}</span>
                  <strong className="mt-2 text-lg font-medium leading-6 text-[#f6efff]">{h('immersive.worlds.sybDiscText')}</strong>
                </div>
              </div>
              <div className="p-7 sm:p-10">
                <p className="font-label text-[10px] uppercase tracking-[0.2em] text-[#c9b2ff]">{h('twoSolutions.syb.badge')}</p>
                <h3 className="mt-3 font-headline text-4xl font-medium tracking-[-0.045em] text-white sm:text-5xl">Soundtrack Your Brand</h3>
                <p className="mt-4 text-lg leading-8 text-[#eee6ff]/58">{h('alternative.description')}</p>
                <ul className="mt-7 flex flex-wrap gap-2">
          ...
          <motion.div {...reveal} className="mt-6 flex flex-col items-start justify-between gap-5 rounded-3xl border border-white/10 bg-white/[0.025] p-6 sm:flex-row sm:items-center sm:p-8">
            <div>
              ...
              <p className="mt-2 text-lg text-white/68">{h('immersive.worlds.compareText')}</p>
            </div>
        ...
        <div className="mx-auto max-w-[1320px]">
          <motion.div {...reveal} className="flex flex-col justify-between gap-8 md:flex-row md:items-end">
            <div className="max-w-2xl">
              ...
              <h2 className="mt-5 font-headline text-4xl font-medium tracking-[-0.045em] text-white md:text-6xl">{h('industries.sectionSubtitle')}</h2>
            </div>
              ...
              <motion.div key={industry.key} {...reveal} transition={{ duration: 0.55, delay: index * 0.08 }}>
                <Link href={`/${locale}${industry.href}`} className="group relative block min-h-[460px] overflow-hidden rounded-[1.5rem] border border-white/10 focus:outline-none focus:ring-2 focus:ring-brand-orange focus:ring-offset-4 focus:ring-offset-[#070d17]">
                  ...
                  <div className="absolute inset-x-0 bottom-0 p-7 md:p-8">
                    ...
                    <h3 className="mt-3 font-headline text-3xl font-medium text-white">{h(`industries.${industry.key}.title`)}</h3>
      ...
      <section id="start" className="relative overflow-hidden border-t border-white/[0.08] bg-[#09111d] px-6 py-24 sm:px-10 md:py-36 lg:px-16">
        ...
        <motion.div {...reveal} className="relative mx-auto max-w-4xl text-center">
          ...
          <h2 className="mt-7 font-headline text-5xl font-medium leading-[1.02] tracking-[-0.05em] text-white md:text-7xl">
            {h('finalCta.title')}{' '}
