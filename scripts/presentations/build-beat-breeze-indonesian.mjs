...
const MOTION_SCRIPT =
  '  <script src="./narration/beat-breeze-motion/controller.js?v=2026-09-03-2" defer></script>';
...
for (const required of requiredFiles) {
  if (!existsSync(required)) {
    throw new Error(`Required Indonesian presentation source is missing: ${required}`);
  }
}

const SLIDE_LABELS = new Map([
  ["Title", "Sampul"],
  ["One platform, many jobs", "Satu platform, banyak fungsi"],
  ["Music that runs itself", "Musik yang berjalan otomatis"],
  ["Automations", "Otomatisasi"],
  ["Your Music Director", "Music Director Anda"],
  ["Compose", "Compose"],
  ["Studio &amp; screens", "Studio dan layar"],
  ["Announcements", "Pengumuman suara"],
  ["Works with Claude &amp; ChatGPT", "Terhubung dengan Claude dan ChatGPT"],
  ["Built for operators", "Dibuat untuk tim operasional"],
  ["Never go silent", "Musik tak pernah terhenti"],
  ["Why Beat Breeze", "Mengapa Beat Breeze"],
  ["Pricing", "Harga"],
  ["Who's behind it", "Tim di balik Beat Breeze"],
  ["Close", "Mulai"],
]);

const INDONESIAN_FONT_CSS = `
section {
  word-break: normal;
...
if (
  ...
  script.presentation !== "beat-breeze-id" ||
  script.language !== "id-ID" ||
  !Array.isArray(script.slides) ||
  script.slides.length !== 15
) {
  throw new Error("The Indonesian presenter script must contain all 15 Beat Breeze slides.");
}
...
for (const [index, match] of sections.entries()) {
  ...
  const indonesianLabel = SLIDE_LABELS.get(englishLabel);
  const scriptSlide = script.slides[index];
  if (!indonesianLabel || scriptSlide.label !== indonesianLabel) {
    throw new Error(`Indonesian label mismatch on slide ${index + 1}: ${englishLabel}.`);
  }

  let section = match[0]
    .replace(`data-label="${englishLabel}"`, `data-label="${indonesianLabel}"`)
    .replace(
      /data-speaker-notes="[^"]*"/,
    ...
    .replace(
      "<section ",
      '<section class="slide" data-zone="slide" data-layout_box_budget="1920x1080-approved-layout-safe-nav-top-940" data-mechanical_layout_preflight="indonesian-copy-and-line-break-check-required" ',
    );
...
template = translatedTemplate
  .replace("<html>", '<html lang="id">')
  .replace(
    "<title>Beat Breeze — Product Overview 2026</title>",
    "<title>Beat Breeze — Ikhtisar Produk 2026</title>",
  )
  .replace("<style>", `<style>${INDONESIAN_FONT_CSS}`);
...
if (untranslated.length) {
  throw new Error(
    `Untranslated Indonesian slide text remains:\n${[
...
let output = source
  ...
  .replace(
    "<title>Beat Breeze — Product Overview 2026</title>",
    "<title>Beat Breeze — Ikhtisar Produk 2026 (Bahasa Indonesia)</title>",
  )
  .replace("Unpacking...", "Menyiapkan presentasi…")
  .replace("This page requires JavaScript to display.", "Presentasi ini memerlukan JavaScript agar dapat ditampilkan.")
  .replace(
  ...
  .replace(
    "\n</body>",
    `\n  <template id="beat-breeze-indonesian-layout-guard-source" aria-hidden="true">\n${[
      ...template.matchAll(/<section\b[^>]*>[\s\S]*?<\/section>/g),
    ]
      .map((match) => match[0])
      .join("\n")}\n  </template>\n</body>`,
  ...
  .replace(
    "\n</body>",
    `\n  <script src="./narration/beat-breeze-id/controller.js" defer></script>\n${MOTION_SCRIPT}\n</body>`,
  )
  .replace(/[ \t]+$/gm, "");
...
console.log(
  `Indonesian presentation built: 15 slides, ${translatedNodeCount} localized text nodes, ${path.relative(
    REPO_ROOT,
    OUTPUT_PATH,
  )}.`,
);
