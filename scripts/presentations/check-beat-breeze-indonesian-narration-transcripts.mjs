...
if (!process.argv.includes("--confirm-spend")) {
  throw new Error(
    "Pass --confirm-spend only for the bounded 15-clip Indonesian transcription QA run.",
  );
}
...
if (script.slides.length !== 15 || manifest.slides.length !== 15) {
  throw new Error("Indonesian transcript QA requires all 15 narration clips.");
}
...
if (failures.length) {
  throw new Error(
    `Indonesian transcript QA failed on: ${failures.map((item) => item.id).join(", ")}`,
  );
}
...
console.log(
  `PASS: 15 Indonesian clips transcribed with mean CER ${(meanCer * 100).toFixed(1)}% and maximum CER ${(maximumCer * 100).toFixed(1)}%.`,
);
