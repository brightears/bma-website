...
const LOCALES = {
  ko: { language: "ko-KR", name: "Korean" },
  ja: { language: "ja-JP", name: "Japanese" },
...
if (
  ...
  throw new Error(
    `The ${locale.name} narration script must contain all 15 Beat Breeze slides.`,
  );
}
...
try {
  ...
  const manifest = {
    ...
    status: `official-${localeCode}-narrated-presentation`,
    ready: true,
    generatedAt,
    releaseId,
    publicationStatus:
      `Official unlinked, noindex ${locale.name} Beat Breeze presentation with complete slide-linked ${locale.name} narration.`,
    coverage: {
    ...
    source: {
      ...
      englishDeckSha256,
      officialDeckPath: `public/presentations/beat-breeze-${localeCode}.html`,
      officialDeckSha256,
      scriptPath:
        `public/presentations/narration/beat-breeze-${localeCode}/script.json`,
      scriptSha256,
      copyPath:
        `public/presentations/narration/beat-breeze-${localeCode}/copy.json`,
      copySha256,
    },
    voice: {
      provider: "google-gemini",
