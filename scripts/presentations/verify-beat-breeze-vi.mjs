...
for (const required of [
  ...
  if (!existsSync(required)) fail(`Required Vietnamese presentation artifact is missing: ${required}`);
}
...
const vietnameseOfficial = readFileSync(VIETNAMESE_OFFICIAL, "utf8");
const script = JSON.parse(readFileSync(SCRIPT_PATH, "utf8"));
const manifest = JSON.parse(readFileSync(MANIFEST_PATH, "utf8"));
const controller = readFileSync(CONTROLLER_PATH, "utf8");

if (
  !vietnameseOfficial.includes('<html lang="vi">') ||
  !vietnameseOfficial.includes("<title>Beat Breeze — Tổng quan sản phẩm 2026 (Tiếng Việt)</title>") ||
  !vietnameseOfficial.includes("./narration/beat-breeze-vi/controller.js")
) {
  fail("The official Vietnamese presentation is missing its language or narration controller.");
}
if (
  vietnameseOfficial.includes("./narration/beat-breeze-voice-preview/controller.js") ||
  vietnameseOfficial.includes("./narration/beat-breeze-voice-preview-th/controller.js") ||
  vietnameseOfficial.includes("./narration/beat-breeze-zh/controller.js")
) {
  fail("The official Vietnamese presentation points to another language's narration controller.");
}
if (
  !vietnameseOfficial.includes("Đội ngũ chuyên gia hỗ trợ 24/7") ||
  /[\u3400-\u9FFF\u0E00-\u0E7F]/.test(controller)
) {
  fail("The Vietnamese slide 14 support wording or controller localization is incorrect.");
}

if (
  manifest.source.englishDeckPath !== "public/presentations/beat-breeze.html" ||
  manifest.source.officialDeckPath !== "public/presentations/beat-breeze-vi.html" ||
  manifest.source.copyPath !==
    "public/presentations/narration/beat-breeze-vi/copy.json" ||
  sha256File(ENGLISH_DECK) !== manifest.source.englishDeckSha256 ||
  sha256File(VIETNAMESE_OFFICIAL) !== manifest.source.officialDeckSha256 ||
  sha256File(SCRIPT_PATH) !== manifest.source.scriptSha256 ||
  sha256File(COPY_PATH) !== manifest.source.copySha256
) {
  fail("A source hash in the Vietnamese narration manifest is stale.");
}

const templateMarker = '<script type="__bundler/template">';
const templateStart = vietnameseOfficial.indexOf(templateMarker) + templateMarker.length;
const templateEnd = vietnameseOfficial.indexOf("</script>", templateStart);
const template = JSON.parse(vietnameseOfficial.slice(templateStart, templateEnd).trim());
...
if (
  ...
  fail("Vietnamese deck, script, and manifest slide labels or order do not match.");
}
if (
  script.language !== "vi-VN" ||
  !script.slides.every((slide) => /[\u0102-\u01B0\u1EA0-\u1EF9]/i.test(slide.text))
) {
  fail("Every Vietnamese narration slide must contain Vietnamese presenter copy.");
}
if (
  manifest.ready !== true ||
  manifest.status !== "official-vietnamese-narrated-presentation" ||
  manifest.slides.length !== 15 ||
  manifest.coverage.slideIndexes.join(",") !==
    Array.from({ length: 15 }, (_, index) => index + 1).join(",")
) {
  fail("The Vietnamese narration manifest does not declare complete 15-slide coverage.");
}
const transcriptionQa = manifest.qualityAssurance?.transcription;
if (
  transcriptionQa?.model !== "gemini-3.5-transcribe" ||
  transcriptionQa?.languageCode !== "vi-VN" ||
  ...
  fail("The Vietnamese narration transcription quality gate is missing or failed.");
}
...
for (const [index, manifestSlide] of manifest.slides.entries()) {
  ...
  if (
    ...
    fail(`Vietnamese media probe mismatch on slide ${index + 1}.`);
  }
  execFileSync("ffmpeg", [
    "-hide_banner",
    "-loglevel",
  ...
  if (
    ...
    fail(`Vietnamese audio loudness gate failed on slide ${index + 1}.`);
  }
  if (!controller.includes(manifestSlide.src)) {
    fail(`Vietnamese local fallback is missing slide ${index + 1}.`);
  }
  durationTotal += manifestSlide.durationSeconds;
}

if (!closeEnough(durationTotal, manifest.totalDurationSeconds, 0.02)) {
  fail("Total Vietnamese narration duration does not match the slide clips.");
}
if (!controller.includes("Quý khách muốn nghe thuyết minh?")) {
  fail("The Vietnamese narration invitation is missing.");
}
if (!controller.includes('<button class="narration-invitation-start" type="button">Bắt đầu</button>')) {
  fail("The Vietnamese narration invitation start label is missing.");
}
if (!controller.includes('visible: "Bắt đầu thuyết minh"')) {
  fail("The Vietnamese initial narration action is not explicit.");
}
...
console.log("PASS: Vietnamese deck preserves 15-slide structure with localized labels and full Vietnamese copy.");
console.log(
  `PASS: ${manifest.slides.length} Vietnamese Sulafat clips decode correctly (${manifest.totalDurationSeconds.toFixed(1)}s total).`,
);
console.log(
  `PASS: automated Vietnamese speech fidelity passed all 15 slides (mean CER ${(transcriptionQa.meanCharacterErrorRate * 100).toFixed(1)}%, max ${(transcriptionQa.maximumCharacterErrorRate * 100).toFixed(1)}%).`,
);
console.log("PASS: Vietnamese deck, script, manifest, audio hashes, labels, and local fallback agree.");
