...
- **Serving** (DNS-verified Aug 12, 2026): bmasiamusic.com is **not on a customer Cloudflare zone**. Nameservers are registrar DNS (domaincontrol.com) pointing straight to Render; the `server: cloudflare` response header comes from **Render's built-in Cloudflare CDN**, which Render manages. There is no Cloudflare dashboard or AI Crawl Control for this domain (Norbert's only Cloudflare zone is beatbreeze.io), and Cloudflare's AI-crawler default changes do not apply here. The effective crawler policy is the repo's `public/robots.txt`. All major AI crawlers (GPTBot, ClaudeBot, PerplexityBot, CCBot, Google-Extended, Bytespider) receive HTTP 200.
...
- **Visible FAQ + FAQPage schema**: `/soundtrack-your-brand`, `/beat-breeze` and `/licensing` each carry a visible FAQ accordion (`components/sections/FaqSection.tsx`, translation namespaces `soundtrackPage.redesign.faq`, `beatBreezePage.redesign.faq`, `licensingPage.faq`) plus FAQPage JSON-LD emitted in the page layout from the same keys. Answers stay mounted in the server-rendered HTML (CSS-only collapse) so non-JS AI crawlers read them; FAQPage schema must always mirror visible on-page Q&A (Google policy) — never add it to a page without a visible FAQ.
- **Known gaps (deliberate/pending)**: no public pricing in HTML (quotation-based business model — AI engines cannot cite prices); `/music-design` is a retired page that redirects to `/quotation` (correctly absent from sitemap).
...
### Aug 11, 2026
- **Visible FAQ sections + FAQPage JSON-LD** on `/soundtrack-your-brand` (8 Q&A), `/beat-breeze` (8) and `/licensing` (7). New shared `components/sections/FaqSection.tsx` accordion (accent-themeable per page, answers always in SSR HTML, CSS grid-rows collapse, WAI-ARIA accordion semantics). Content grounded in existing site copy and the llms.txt buyer Q&A; translated to all 8 locales via 7 parallel Opus translator subagents; FAQPage schema emitted in each page layout from the same translation keys.
