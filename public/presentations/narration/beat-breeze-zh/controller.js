(() => {
  ...
  const CONTROLLER_URL =
    ...
    new URL(
      "./narration/beat-breeze-zh/controller.js",
  ...
  const INVITATION_SESSION_KEY =
    "beat-breeze-narration-invitation-seen-zh";
  ...
  const LOCAL_FALLBACK_MANIFEST = {
    ...
    slides: [
      {
        index: 1,
        src: "audio/20260902-59014dd083de/01-title.mp3",
      },
      {
        index: 2,
        src: "audio/20260902-59014dd083de/02-one-platform-many-jobs.mp3",
      },
      {
        index: 3,
        src: "audio/20260902-59014dd083de/03-music-that-runs-itself.mp3",
      },
      {
        index: 4,
        src: "audio/20260902-59014dd083de/04-automations.mp3",
      },
      {
        index: 5,
        src: "audio/20260902-59014dd083de/05-your-music-director.mp3",
      },
      {
        index: 6,
        src: "audio/20260902-59014dd083de/06-compose.mp3",
      },
      {
        index: 7,
        src: "audio/20260902-59014dd083de/07-studio-and-screens.mp3",
      },
      {
        index: 8,
        src: "audio/20260902-59014dd083de/08-announcements.mp3",
      },
      {
        index: 9,
        src: "audio/20260902-59014dd083de/09-works-with-claude-and-chatgpt.mp3",
      },
      {
        index: 10,
        src: "audio/20260902-59014dd083de/10-built-for-operators.mp3",
      },
      {
        index: 11,
        src: "audio/20260902-59014dd083de/11-never-go-silent.mp3",
      },
      {
        index: 12,
        src: "audio/20260902-59014dd083de/12-why-beat-breeze.mp3",
      },
      {
        index: 13,
        src: "audio/20260907-pricing-24e85ed7d14f093a/13-pricing.mp3",
      },
      {
        index: 14,
        src: "audio/20260902-59014dd083de/14-whos-behind-it.mp3",
      },
      {
        index: 15,
        src: "audio/20260902-59014dd083de/15-close.mp3",
      },
    ],
  };
  ...
  const loadManifest = async () => {
    ...
    if (!manifestPromise) {
      manifestPromise = fetch(MANIFEST_URL, { cache: "no-store" })
        .then((response) => {
          if (!response.ok) {
            throw new Error(`讲解清单返回状态 ${response.status}`);
          }
          return response.json();
        })
        .then((value) => {
          if (!value || value.deck !== "beat-breeze") {
            throw new Error("讲解清单与此演示不匹配");
          }
          manifest = value;
          return value;
        })
  ...
  const setNarrationUnavailable = () => {
    ...
    label.textContent = "本页无讲解";
    shortLabel.textContent = "无讲解";
    button.setAttribute(
      "aria-label",
      "本页没有讲解音频",
    );
    button.title = "本页没有讲解音频";
  };
  ...
  const playCurrentSlide = async ({ restart = false } = {}) => {
    ...
    try {
      ...
      announce(`正在播放第 ${currentIndex + 1} 页讲解`);
      await audio.play();
    } catch (error) {
      if (request !== playbackRequest || error?.name === "AbortError") return;
      console.error("无法播放 Beat Breeze 讲解", error);
      failPlayback("无法播放讲解，请重试");
    }
  };
  ...
  const install = () => {
    ...
    toolbar.setAttribute("aria-label", "演示控制栏");
    ...
    if (resetControl) {
      resetControl.setAttribute("aria-label", "重新开始演示");
      resetControl.title = "重新开始演示";
    }
    ...
    button.innerHTML = `
      ...
      <span class="narration-label">讲解</span>
      <span class="narration-label-short">声音</span>
    `;
    ...
    invitation.setAttribute("aria-label", "开始讲解邀请");
    invitation.innerHTML = `
      <span class="narration-invitation-copy">想听讲解版吗？</span>
      <button class="narration-invitation-start" type="button">开始讲解</button>
      <button class="narration-invitation-dismiss" type="button" aria-label="关闭讲解邀请">×</button>
    `;
    ...
    audio.addEventListener("ended", () => {
      ...
      if (!isFinalNarratedSlide(currentIndex) && nextSlideHasNarration) {
        ...
        announce("带讲解的演示已播放完毕");
      }
    });
    audio.addEventListener("error", () => {
      if (audio.getAttribute("src")) {
        failPlayback("无法加载讲解音频");
      }
    });
    ...
    void loadManifest()
      ...
      .catch((error) => {
        console.error("无法加载 Beat Breeze 讲解清单", error);
        setButtonState("error");
        announce("无法加载讲解，请重试");
      });
